...
- Max: вход по телефону/SMS из вкладки Чаты, сессия в `kid_max_session`, история через `GET /api/kids/max-sync` (при открытии Чаты + cron раз в час). Локальный демон не нужен.
